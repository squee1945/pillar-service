...
func createCloudBuildTool(githubToken, projectID, region, subBuildServiceAccount, subBuildLogsBucket string) mcp.ToolHandlerFor[createCloudBuildInput, createCloudBuildOutput] {
	return func(ctx context.Context, req *mcp.CallToolRequest, input createCloudBuildInput) (*mcp.CallToolResult, createCloudBuildOutput, error) {
		...
		if err := protojson.Unmarshal([]byte(input.CloudBuildJSON), &build); err != nil {
			return nil, createCloudBuildOutput{}, fmt.Errorf("json.Unmarshal: %w", err)
		}
		...
		if err != nil {
			return nil, createCloudBuildOutput{}, fmt.Errorf("marshalling build: %w", err)
		}
		...
		if build.Artifacts == nil || len(build.Artifacts.GoModules) == 0 {
			return nil, createCloudBuildOutput{}, errors.New("missing required artifacts.goModules block")
		}
...
func fetchTestOutputTool(subBuildTestOutputBucket string) mcp.ToolHandlerFor[fetchTestOutputInput, fetchTestOutputOutput] {
	return func(ctx context.Context, req *mcp.CallToolRequest, input fetchTestOutputInput) (*mcp.CallToolResult, fetchTestOutputOutput, error) {
		if err := input.validate(); err != nil {
			return nil, fetchTestOutputOutput{}, err
		}
		storageClient, err := storage.NewClient(ctx)
		if err != nil {
			return nil, fetchTestOutputOutput{}, fmt.Errorf("storage.NewClient: %w", err)
		}
		defer storageClient.Close()

		bucket := strings.TrimPrefix(subBuildTestOutputBucket, "gs://")
		object := input.Filename

		rc, err := storageClient.Bucket(bucket).Object(object).NewReader(ctx)
		if err != nil {
			return nil, fetchTestOutputOutput{}, fmt.Errorf("getting logs object (bucket %q object %q): %w", bucket, object, err)
		}
		defer rc.Close()
...
func fetchProvenanceTool(projectID, region string) mcp.ToolHandlerFor[fetchProvenanceInput, fetchProvenanceOutput] {
	return func(ctx context.Context, req *mcp.CallToolRequest, input fetchProvenanceInput) (*mcp.CallToolResult, fetchProvenanceOutput, error) {
		if err := input.validate(); err != nil {
			return nil, fetchProvenanceOutput{}, err
		}
		...
		if build.GetStatus() != cloudbuildpb.Build_SUCCESS {
			return nil, fetchProvenanceOutput{}, errors.New("build status must be SUCCESS to fetch provenance")
		}
		...
		if err != nil {
			return nil, fetchProvenanceOutput{}, fmt.Errorf("containeranalysis.NewClient: %w", err)
		}
		defer client.Close()
		...
		for {
			...
			if err != nil {
				return nil, fetchProvenanceOutput{}, fmt.Errorf("iterating occurrences: %w", err)
			}
		...
		for _, occ := range occurrences {
			...
			if err != nil {
				return nil, fetchProvenanceOutput{}, fmt.Errorf("marshalling occurence: %w", err)
			}
